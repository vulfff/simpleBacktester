...
class BacktestEngine:
	...
	def _handle_tick(self, event: TickEvent) -> None:
		...
		mid_price = (tick.bid + tick.ask) / 2 if tick.ask else tick.bid
		self.portfolio.update_market_price(tick.name, mid_price)
	...
	def _handle_order(self, event: OrderEvent) -> None:
		...
		if tick is None:
			return
		price = tick.ask if event.action == "buy" else tick.bid
		fill = FillEvent(
			symbol=event.symbol,
			action=event.action,
